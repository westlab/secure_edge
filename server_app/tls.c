...
int key_out(TLSParameters *tls_context)
{
  FILE *fp;
  char filepath[256];
  //sprintf(filepath,"/home/hiroki/implementing-ssl/after.new/server_app/keyfile/key0.dat",key_num);
  sprintf(filepath,"./keyfile/key0.dat",key_num);
  ProtectionParameters *parameterre;
  SubParameters *parameter;
  parameter= malloc(sizeof(SubParameters));
  memset(parameter,0x0,sizeof(SubParameters));
  parameterre = &tls_context->active_send_parameters;
  CipherSuite *active_suite;
  active_suite = &suites[ parameterre->suite ];
  memcpy(&parameter->MAC_secret,parameterre->MAC_secret,active_suite->hash_size);
  memcpy(&parameter->key,parameterre->key,active_suite->key_size);
  memcpy(&parameter->IV,parameterre->IV,active_suite->IV_size);
  
  parameter->suite=parameterre->suite;
  parameter->seq_num=parameterre->seq_num;
 /*  fprintf(stderr,"mac_secret  is %x\n",parameterre->MAC_secret);
  fprintf(stderr,"mac_secret  is %x\n",parameter->MAC_secret);
  fprintf(stderr,"key is %x\n",*parameterre->key);
  fprintf(stderr,"key is %x\n",*parameter->key);
  fprintf(stderr,"IV is %x\n",*parameterre->IV); 
  fprintf(stderr,"IV is %x\n",*parameter->IV); 
  fprintf(stderr,"suite size is  %d\n",sizeof(CipherSuiteIdentifier));
  fprintf(stderr,"seq is %ld\n",sizeof(parameterre->seq_num)); */

  /* FILE *fb;
  void *str[40]={};
  memcpy(str,parameterre->MAC_secret,8);
  memcpy(str+8,parameterre->key,8);
  memcpy(str+16,parameterre->IV,8);
  memcpy(str+24,&parameterre->suite,8);
  memcpy(str+32,&parameterre->seq_num,8);
  fb =fopen("key3.dat","wb");
  fwrite(str,sizeof(str),1,fb);
  fclose(fb); */
  ...
  fwrite(parameter,sizeof(*parameter),1,fp);
...
int file_out(unsigned char *buf, int bufsz,int i)
{
  char filepath[64]={};
  sprintf(filepath,"%dbuffer.dat",i);
...
void read_file(SubParameters *parameter,int bufsz,TLSPlaintext*header, unsigned char *encrypt,int i)
{ 
  ProtectionParameters *prepara; 
  SubParameters *par;
  prepara =malloc(sizeof(*prepara));
  par =malloc(sizeof(*par));
  FILE *fp,*fb;
  unsigned char *preheader, *preencrypt;
  char filepath[64];
  sprintf(filepath,"%dbuffer.dat",i);
  preheader = (unsigned char *)malloc(5);
  preencrypt= (unsigned char *)malloc(bufsz-5);
  
  
  

  fp=fopen("key.dat","rb");
  fb=fopen(filepath,"rb");
  
  fread(par,sizeof(*par),1,fp);
  fread(header,5,1,fb);
  fread(encrypt,bufsz-5,1,fb);

  /* prepara->MAC_secret=&par->MAC_secret;
  prepara->key=&par->key;
  prepara->IV=&par->IV;
  prepara->seq_num=par->seq_num;
  prepara->suite=par->suite; */ 

  /* memcpy(prepara->MAC_secret,&par->MAC_secret,20);
  memcpy(prepara->key,&par->key,24);
  memcpy(prepara->IV,&par->IV,8);
  prepara->seq_num=par->seq_num;
  prepara->suite=par->suite; 
   */
  
  *parameter=*par;
  /* header->type=preheader[0];
  header->version.major=preheader[1];
  header->version.minor=preheader[2];
  header->length=bufsz-5; */
...
{ 
  TLSPlaintext header;
  ProtectionParameters *par2;
  unsigned char *send_buffer,*send_buffer2;
...
if (send_buffer_size>100) file_out(send_buffer,send_buffer_size,5);
...
/* FILE *af,*bf;
af =fopen("buffer1.dat","a");
bf =fopen("buffer2.dat","a");
fprintf(af,"%d\n",send_buffer_size);
fprintf(bf,"%d\n",content_len);
fclose(af);
fclose(bf); */
...
if(send_buffer_size>100)
{
 if (file_out(send_buffer,send_buffer_size,1))
  {
    printf("refused file out ");
  }
}
  ...
  /* unsigned char *header2,*encrypt2,*decrypt2;
  read_file(par2,send_buffer_size,header2,encrypt2);
  tls_decrypt(header2,encrypt2,send_buffer_size-5,&decrypt2,par2);
  printf (" send message is %s",decrypt2);
  printf("\n"); */
...
int key_send_message( int connection,
                         int content_type,
                         const unsigned char *content,
                         short content_len,
...
static void remember_session( TLSParameters *parameters )
{
  if ( parameters->session_id_length > 0 )
  {
    ...
    fprintf(stderr,"session is(remenmber_session) %d\n",session_id);
    if ( head == NULL )
    {
      head = stored_sessions[ session_id % HASH_TABLE_SIZE ] =
...
static void find_stored_session( int session_id_length,
                                 ...
                                 TLSParameters *parameters )
{
  int session_id_num=NULL;
  StoredSessionsList *head;
  char master;
  //fprintf(stderr,"serching session id\n");
  ...
  //fprintf(stderr,"amari is %x  %d\n",session_id_num,(session_id_num % HASH_TABLE_SIZE) );
 
  for ( head = stored_sessions[ session_id_num % HASH_TABLE_SIZE ];
        head != NULL;
        head = ( StoredSessionsList * ) head->next )
  {
    memcpy(&master, head->master_secret,5 );
    fprintf(stderr,"head liest is %s - %d\n",head->session_id,master);
...
{ 
  ...
  if ( parameters->session_id_length == 0 )
  {
    ...
    int s_id;
    memcpy(&s_id,parameters->session_id,parameters->session_id_length);
    fprintf(stderr,"Session_id(send server hello le=0) is %d\n",s_id);
  }
 //fprintf(stderr,"Session_id_length is %d\n",parameters->session_id_length);
  package.session_id_length = parameters->session_id_length;
  //package.session_id[0] = parameters->session_id;//kokogayokuwakaran
  memcpy( &package.session_id, parameters->session_id, sizeof( int ) );
                                                 ...
                                                 TLSParameters *parameters )
{
  ...
  {
    perror( "Unable to load file(key.pkcs8(der))" );
    return 0;
  } 
                                    ...
                                    TLSParameters *parameters )
{
  ...
  if ( !( buffer = load_file_into_memory( "key.der", &buffer_length ) ) )
  {
    perror( "Unable to load file(key.der)" );
    return 0;
  }
...
static int tls_decrypt2( const unsigned char *header, // needed for MAC verification
                        unsigned char *encrypted_message,
                        short encrypted_length,
                        unsigned char **decrypted_message, 
                        SubParameters *parameters )
...
int key_receive_tls_msg( int connection,
                            char *buffer,
                            int bufsz,	
                            TLSParameters *parameters )
{
...
int s_id;
memcpy(&s_id,parameters->session_id,parameters->session_id_length);
//fprintf(stderr,"session_id(tls_accept1) is %d \n",s_id);
//fprintf(stderr,"Session_id_length1 is %d\n",parameters->session_id_length);
  ...
  memcpy(&s_id,parameters->session_id,parameters->session_id_length);
 //fprintf(stderr,"session_id(tls_accept2) is %d \n",s_id);
 //fprintf(stderr,"Session_id_length2 is %d\n",parameters->session_id_length);
  if ( parameters->session_id_length > 0 )
  {
...
{
  /* if(length>100){
    file_out(application_data,length,0);
  } */
  ...
  short send_buffer_size;
  int padding_length=0;
  send_buffer_size =length + SHA1_BYTE_SIZE;
  if((send_buffer_size%8)!=0){
    padding_length = 8 -( send_buffer_size % 8 );
    send_buffer_size += padding_length;
  }
  send_buffer_size+=5;
  ...
  encrypt2 =(unsigned char *)malloc(send_buffer_size-5);
  decrypt2 =(unsigned char *)malloc(send_buffer_size-5);
  read_file(par2,send_buffer_size,header2,encrypt2,1);
  send_buffer_size = tls_decrypt2(header2,encrypt2,send_buffer_size-5,&decrypt2,par2);
  decrypt2[send_buffer_size]='\0';
  printf ("%s",decrypt2);
  printf("\n");
  file_out(decrypt2,send_buffer_size,6);
  free(par2); 
  free(header2);
  free(encrypt2);
  free(decrypt2); */
