...
for(int i; i<=5; i++){
  //fprintf(stderr,"send_buffer[%d] is %d ",i,send_buffer[ i ]);
}
fprintf(stderr,"\n");
                              ...
                              int renegotiating )
{
 ...
 //fprintf(stderr,"session_id_length(hello) is %d \n",parameters->session_id_length);
  if ( parameters->session_id_length > 0 )
  {
  ...
  supported_suites[ 0 ] = htons(  TLS_RSA_WITH_AES_256_CBC_SHA );
  ...
  write_buffer = append_buffer( write_buffer, ( void * )  
     ...
     //fprintf(stderr,"package.session_id_length is %d\n",package.session_id_length);
...
//fprintf(stderr,"session_id(tls_resume) is %s \n",parameters->session_id);
//fprintf(stderr,"session_id_length(tls_resume) is %d \n",parameters->session_id_length);
  new_md5_digest( &parameters->md5_handshake_digest );
  new_sha1_digest( &parameters->sha1_handshake_digest );
  ...
  while ( !parameters->peer_finished )
  {
    ...
    if ( server_hello_done )
    {
      ...
      //fprintf(stderr,"reqsession_id is %s\n session_id is %d \n",session_id,s_id);
